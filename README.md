# Reinforcement learning of transferable quantum circuit architectures for generating potential energy curves

### Abstract
Variational quantum algorithms (VQAs) represent a promising method for solving problems in areas such as optimization and quantum chemistry using near-term quantum hardware. However, the design of the quantum circuit ansatz, which highly influences its performance, remains a challenge. Performance issues arising from
problem-agnostic ansätze, such as barren plateaus, mean that quantum circuits for VQAs must be problem-tailored. Here, we are particularly interested in developing circuits that are not merely tailored to a specific problem, as defined by some Hamiltonian, but to a family of Hamiltonians that define a class of problems.
To this end, we present a reinforcement learning (RL) approach for quantum circuit mapping that outputs a circuit for each instance of a given family of molecular Hamiltonians. Specifically, our input consists of a molecule and a discrete set of bond distances, and the output is a bond-distance-dependent quantum circuit that is transferable across the potential energy curve and thereby enables generalization to arbitrary bond distances. The inherently non-greedy approach of our RL method contrasts with existing greedy approaches to adaptive, problem-tailored circuit constructions. We demonstrate its effectiveness for the four-qubit and six-qubit lithium hydride molecules, as well as an eight-qubit H4 chain. Furthermore, we demonstrate that the learned mapping and circuits are interpretable in a physically meaningful manner, paving the way for applying our insights from RL-inspired quantum circuits to small-scale molecules to develop novel quantum circuits for the ground states of large-scale molecular systems. 
